...
> Maintained fork of [turakvlad/replace-color](https://github.com/turakvlad/replace-color) with bug fixes, dependency modernisation, and new features.

Replaces a colour with another one pixel by pixel using Delta-E perceptual colour difference. Especially useful for removing watermarks from images. Built on top of [Jimp](https://github.com/oliver-moran/jimp).

## Install

```sh
npm install --save @bondybondbond/recolour
```

## Basic usage

`recolour` supports both Node.js error-first callbacks and promises. The package returns a Jimp instance which you can use to execute other [image manipulation methods](https://github.com/oliver-moran/jimp#image-manipulation-methods) or save with Jimp's [`write`](https://github.com/oliver-moran/jimp/tree/master/packages/jimp#writing-to-files-and-buffers) method.

### Node.js error-first callback example

```javascript
const replaceColor = require('@bondybondbond/recolour')
...
* `options` *[Object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) (required)* - the options.
  * `image` *[Buffer](https://nodejs.org/api/buffer.html#buffer_class_buffer) | [Object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) | [String](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String) (required)* - an image being processed. It can be a **buffer**, **Jimp's instance**, a **path to an image on your host machine** or a **URL address to an image on the internet**. Please, take a look at the [tests](https://github.com/bondybondbond/recolour/tree/master/test/recolour.js) to understand all these options.
  * `colors` *[Object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) | [Array](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array) (required)* - the colors. Pass an array to replace multiple colours in a single pass.
  ...
  * `deltaE` *[Number](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number) (optional)* - a `deltaE` value which corresponds to a [JND](https://en.wikipedia.org/wiki/Just-noticeable_difference). The default value is `2.3`. Please, read more about `deltaE` [here](http://zschuessler.github.io/DeltaE/learn/). Generally speaking, if the processed image still has watermarks, you should increase the `deltaE` value.
...
To indicate recolour errors you should use the `err instanceof replaceColor.ReplaceColorError` class.

```javascript
replaceColor({}, (err, jimpObject) => {
  if (err instanceof replaceColor.ReplaceColorError) {
    // A recolour error occurred.
  } else if (err) {
