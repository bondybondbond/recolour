...
### Added
- **Region resize handles** (`web/index.html`, `web/styles.css`, `web/app.js`): after drawing a selection box, 8 drag handles (4 corners + 4 edge midpoints) appear on the committed overlay. Dragging any handle resizes the box live — the recolour/smart-fill preview re-runs within the new bounds on each animation frame (rAF-throttled). Escape mid-drag restores the pre-drag bounds. Handles are hidden while drawing a new box or in select mode. Hit target is larger than the visual dot (padded) to match pro-editor ergonomics. Closes #34.
- **"Match shades" / "Exact match" toggle** (`web/index.html`, `web/styles.css`, `web/app.js`): the previously inert "Include shades" toggle is now wired as an exact/fuzzy gate. OFF = exact hex match only (tolerance slider dimmed and forced to 0, label reads "Exact match"). ON = tolerance slider active, label reads "Match shades". Default ON. Useful for flat-colour graphics where only a single specific hex should be targeted without catching gradients or anti-aliased edges.
...
### Fixed
- **Portrait-image canvas scaling** (`web/styles.css`): portrait images were displayed with severely non-uniform scaling (sx up to 3.4× sy), making the eyedropper, region drag, and resize handles feel inaccurate. Fixed by constraining the canvas with `max-width` + `max-height` + `width/height: auto` so the browser always derives both dimensions from the intrinsic aspect ratio, guaranteeing sx === sy. Closes #33.
- **Loupe shows live preview result** (`web/app.js`): while a recolour/smart-fill preview was active, the loupe and mid-preview eyedropper pick sampled `baseImageData` (the last committed state) rather than the in-progress result displayed on canvas. `renderPreview()` now stores the work ImageData in `livePreviewImageData`; the loupe reads `livePreviewImageData || baseImageData`. Closes #36.
